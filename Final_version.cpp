...
GLfloat light_position[] = {0, 4,0.40, 0};  // Posicione a lâmpada acima da mesa
	...
	// Variáveis globais para iluminação  nao usadas
...
int NUMERO_TECLAS = 17;
	...
	void cama() {
			...
			GLfloat panoVermelhoEspecular[] = {0.2, 0.0, 0.0, 1.0};  // Cor especular do pano vermelho
			GLfloat brilho = -3;                                   // Brilho do material
	...
	void boneco() {
	    ...
	    GLfloat corEspecular[] = {0.2, 0.2, 0.2, 1.0};    // Cor especular do boneco
	...
	void violao2(){
 			...
 			GLfloat madeiraAmbiente[] = {0.4, 0.1, 0.1, 1.0};   // Cor ambiente da madeira
			GLfloat madeiraDifusa[] = {0.3, 0.0, -0.8, 1.0};     // Cor difusa da madeira
			GLfloat madeiraEspecular[] = {0.3, 0.3, 0.3, 1.0};  // Cor especular da madeira
			GLfloat brilho = 0;                              // Brilho do material
	...
	void tocaMusica(unsigned char key, int x, int y) {
	    ...
	    switch (key) {
			...
			case 'K':
				...
				parametrosVisualizacao();// Inverte o estado da luz 2
